...
def encryptiontest(numoftests):
   
    # do not change this variables
    assert len(corechainex.encrypt(str(datetime.datetime.now()), 1, "1", "")[0]) >= 10, "No result encryption expected. "
    assert len(corechainex.encrypt(str(datetime.datetime.now()), 1, "1", "")) == 1, "Two values given, 1 expected"
    assert len(corechainex.encrypt(str(datetime.datetime.now()), 1, "1", "I want 2 values")) == 2, "No two values given, 2 expected"
    numoftests = 3
    for i in range(100):
    ...
    return numoftests
print("ALL TESTS PASSED:", encryptiontest(numoftests))